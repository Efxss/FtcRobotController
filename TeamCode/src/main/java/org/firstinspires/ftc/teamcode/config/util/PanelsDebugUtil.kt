...
class PanelsDebugUtil(
    ...
    fun showAllDebugTeleop(
        ...
        panels?.apply {
            ...
            debug("Follower Pose X", follower.pose.x)
            debug("Follower Pose Y", follower.pose.y)
            debug("Follower Drive Error", follower.driveError)
            debug("Follower Translation Error", follower.translationalError)
            debug("Follower Heading Error", follower.headingError)
            debug("Follower Error Calculation", follower.errorCalculator)
            debug("Follower Heading", Math.toDegrees(follower.pose.heading))
            debug("Total Heading", Math.toDegrees(follower.totalHeading))
            debug("")
            debug("=== Vision ===")
            debug("Rotate Power", limelight.getRotationPowerFromTag(alliance))
            debug("is Seen", limelight.isTagSeen(alliance))
