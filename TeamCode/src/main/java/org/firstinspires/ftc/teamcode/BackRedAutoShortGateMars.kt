...
@Autonomous(name = "Back Red Auto (9 ball gate Mars)", group = "Main Red")
class BackRedAutoShortGateMars : OpMode() {
    var panels: TelemetryManager? = null
    val scope = CoroutineScope(Dispatchers.Default + SupervisorJob())
    var runDetections: Job? = null
    var outTakeCalc: Job? = null
    var correction: Job? = null
    var autoCanel: Job? = null
    ...
    override fun start() {
        ...
        /*autoCanel = scope.launch {
            while (isActive) {
                if (pathState >= 13 && follower.pose.y >= 14) {
    ...
    fun autonomousPathUpdate() {
        ...
        when (pathState) {
            ...
            8 -> {
                /*if (correction?.isActive != true) {
                    correction = scope.launch {
                        while (isActive) {
                            if (follower.pose.y < 12.0) sleep(20000)
                            delay(15)
                        }
                    }
                }*/
            ...
            13 -> {
                ...
                /*correction = scope.launch {
                    while (isActive) {
                        if (follower.pose.y < 12.0) sleep(20000); panels?.debug("Stop ran"); panels?.update(telemetry)
                        delay(2)
                    }
                }*/
                    /*if (notBusy) {
    ...
    fun buildPaths() {
        ...
        leavePoint = follower.pathBuilder()
            .addPath(BezierLine(spikeScore2, gatePos))
