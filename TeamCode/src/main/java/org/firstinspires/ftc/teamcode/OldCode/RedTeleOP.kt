...
class RedTeleOP : OpMode() {
    ...
    override fun loop() {
        ...
        if (isCentering) {
            ...
            follower.setMaxPower(if (isSlowMode) 0.3 else 0.6)
        ...
        val slowModeButtonPressed = gamepad1.left_bumper && gamepad1.circle
    ...
    private fun runTelemetry() {
        ...
        // --- Pose & Movement ---
        panels?.debug("=== POSE ===", "")
        panels?.debug("X", String.format("%.2f", follower.pose.x))
        panels?.debug("Y", String.format("%.2f", follower.pose.y))
        panels?.debug("Heading (deg)", String.format("%.1f", Math.toDegrees(follower.pose.heading)))

        // --- State Machine ---
        panels?.debug("=== STATE ===", "")
        panels?.debug("Dispensing State", dispensingState)
        panels?.debug("Load Position", currentLoadPosition)
        panels?.debug("Is Centering", isCentering)
        panels?.debug("Is Slow Mode", isSlowMode)
        panels?.debug("Endgame Mode", EndGame.ISENDGAME)

        // --- Game Piece Orders ---
        panels?.debug("=== ORDERS ===", "")
        panels?.debug("Current Order", currentOrder.toString())
        panels?.debug("Expected Order", expectedOrder.toString())
        panels?.debug("Order Full", currentOrder.isFull())
        panels?.debug("Order Empty", currentOrder.isEmpty())
        panels?.debug("Orders Match", currentOrder.matches(expectedOrder))

        // --- Motors ---
        panels?.debug("=== MOTORS ===", "")
        panels?.debug("OutTake1 Power", String.format("%.3f", outTake1.power))
        panels?.debug("OutTake2 Power", String.format("%.3f", outTake2.power))
        panels?.debug("OutTake1 Velocity", String.format("%.1f", outTake1.velocity))
        panels?.debug("OutTake2 Velocity", String.format("%.1f", outTake2.velocity))
        panels?.debug("OutTake Calc Speed", String.format("%.3f", DepoCenter.OUTTAKE_SPEED))
        panels?.debug("Lift Left Pos", liftLeft.currentPosition)
        panels?.debug("Lift Right Pos", liftRight.currentPosition)
        panels?.debug("Lift Left Power", String.format("%.2f", liftLeft.power))
        panels?.debug("Lift Right Power", String.format("%.2f", liftRight.power))

        // --- Servos ---
        panels?.debug("=== SERVOS ===", "")
        panels?.debug("Bowl Servo", String.format("%.3f", bowlServo.position))
        panels?.debug("Cam Servo", String.format("%.3f", camServo.position))
        panels?.debug("Intake1 Power", String.format("%.1f", intakeServo1.power))
        panels?.debug("Intake2 Power", String.format("%.1f", intakeServo2.power))

        // --- Vision ---
        panels?.debug("=== VISION ===", "")
        val llResult = limelight.latestResult
        panels?.debug("Limelight Valid", llResult != null)
        panels?.debug("LL Staleness (ms)", llResult?.staleness ?: -1)
        val pyOutput = llResult?.pythonOutput
        panels?.debug("Python Output Size", pyOutput?.size ?: 0)

        val atDetections = tagProcessor?.detections.orEmpty()
        panels?.debug("AprilTag Count", atDetections.size)
        panels?.debug("AprilTag IDs", atDetections.map { it.id }.toString())

        // --- Coroutine Status ---
        panels?.debug("=== COROUTINES ===", "")
        panels?.debug("outTakeCalc", outTakeCalc?.isActive ?: false)
        panels?.debug("manualFire", manualFire?.isActive ?: false)
        panels?.debug("actVision", actVision?.isActive ?: false)
        panels?.debug("patDect", patDect?.isActive ?: false)
        panels?.debug("actLift", actLift?.isActive ?: false)

        // --- Gamepad Inputs ---
        panels?.debug("=== INPUTS ===", "")
        panels?.debug("Cross", gamepad1.cross)
        panels?.debug("Circle", gamepad1.circle)
        panels?.debug("Square", gamepad1.square)
        panels?.debug("Triangle", gamepad1.triangle)
        panels?.debug("L-Bumper", gamepad1.left_bumper)
        panels?.debug("R-Bumper", gamepad1.right_bumper)
        panels?.debug("L-Trigger", String.format("%.2f", gamepad1.left_trigger))
        panels?.debug("R-Trigger", String.format("%.2f", gamepad1.right_trigger))

        // --- Timing ---
        panels?.debug("=== TIMING ===", "")
        panels?.debug("OpMode Time (s)", String.format("%.1f", opmodeTimer.elapsedTimeSeconds))
