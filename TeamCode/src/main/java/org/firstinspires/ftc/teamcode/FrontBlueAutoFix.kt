...
@Autonomous(name = "Front Blue Auto (12 ball gate Fix)", group = "Main Blue")
class FrontBlueAutoFix : OpMode() {
    ...
    private val strafeOut    = Pose(20.0,  70.0,   Math.toRadians(0.0))
        ...
        when (pathState) {
            ...
            4 -> {
                if (notBusy) {
                    ...
                    follower.followPath(spike1Score)
                    setPathState(5)
                }
            }
            5 -> {
            ...
            14 -> {
                if (!isDispensing) follower.followPath(leavePoint); setPathState(15)
            }
        }
    }
    ...
    fun handleIntake() {if (isDispensing) {
